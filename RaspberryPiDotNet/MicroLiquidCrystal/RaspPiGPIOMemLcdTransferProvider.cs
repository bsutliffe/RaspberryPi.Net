...
namespace RaspberryPiDotNet.MicroLiquidCrystal {
	...
	public class RaspPiGPIOMemLcdTransferProvider : IDisposable, ILcdTransferProvider {
		...
		public RaspPiGPIOMemLcdTransferProvider(bool fourBitMode, GPIOPins rs, GPIOPins rw, GPIOPins enable,
			...
			GPIOPins[] dataPins = new GPIOPins[] { d0, d1, d2, d3, d4, d5, d6, d7 };
		...
		public void Send(byte value, bool mode, bool backlight, bool highOnly = false) {
			if (_disposed)
		...
		private void Write8Bits(byte value) {
			...
			for (int i = 0; i < 8; i++) {
				bool val = ((value >> i) & 0x01) == 0x01;
				_dataPorts[i].Write(val);
				vals.Add(val);
			}
			vals.Reverse();
			Console.WriteLine(string.Join("", vals.Select(v => v ? "1" : "0")));
